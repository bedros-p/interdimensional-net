...
function constructAIPrompt(concept: string, path: string): string {
  return `
{CONCEPT - ${concept}}

Use the concept literally - not always as a region. Do as you see fit. But it should be treated literally most of the time, instead of a simple name of a site - it should be the concept of the site.
...
(HTML should be styled [ you can use bootstrap, it's all bundled in. site gotta be in dark mode though, so make sure all text is readable] - your view is in the "body>main" tag. do not modify width & height of body, or the layout. For layout, use your container.) Use Bootstrap classes extensively for layout, components (like navbars, cards, buttons), and typography to create a modern and responsive design.
It's dark mode, so make sure the text is white & readable on a dark bg`;
}

const handleApi = (c: Context) => {
    c.status(200);
    return c.text('API endpoint: ' + c.req.path);
};

const handleLogo = (c: Context) => {
    c.header('Content-Type', 'image/png');
    c.status(200);
    return c.body(logo);
};

const handleFavicon = (c: Context) => {
    c.header('Content-Type', 'image/png');
    c.status(200);
    return c.body(favicon);
};

const handleEmptyPng = (c: Context) => {
    c.header('Content-Type', 'image/png');
    c.status(200);
    return c.body(emptyPng);
};

const handleMain = async (c: Context) => {
  ...
  const prompt = constructAIPrompt(concept, path);
...
api.get('/:endpoint{.*}', handleApi);

templateServer.get('/assets/logo.png', handleLogo);
templateServer.get('/assets/favicon.png', handleFavicon);
templateServer.get('*.png', handleEmptyPng);
templateServer.get('*', handleMain);
